<script lang="ts">
  ...
  // Initialize state from existing page data
  let pageId = data.page.id;
  let pageSlug = $state(data.page.slug);
  ...
  let featuredImageId = $state(data.page.featuredImageId || null);
  ...
  let publicationDate = $state(
    data.page.publishedAt 
      ? new Date(data.page.publishedAt).toISOString().split('T')[0]
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const pageData = {
        ...
        slug: pageSlug, // Keep existing slug unless changed
      ...
      if (response.ok) {
        ...
        console.log('✅ Updated page data:', result.page);
...
<div class="cms-page-header">
  <div>
    <h1 class="cms-page-title">{m.pages_edit_page()}</h1>
    <p class="cms-page-subtitle">{m.pages_edit_subtitle()}</p>
  </div>
          ...
          <div>
            ...
            <div class="relative">
              ...
              >
                ...
                <option value="archived">{m.pages_status_archived()}</option>
                <option value="trash">{m.pages_status_trash()}</option>
              </select>
            </div>
          </div>
