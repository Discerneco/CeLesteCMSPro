...
async function testDatabase() {
  ...
  try {
    ...
    const db = drizzle(sqlite, { schema: { users, posts, categories, tags, contentTypes, postCategories, postTags } });
    ...
    console.log(`- Tags: ${existingTags.length}\n`);
    ...
    if (existingUsers.length > 0 && existingPosts.length < 4) {
      console.log('🌱 Adding sample posts...\n');
      await createSamplePosts(db, existingUsers[0], existingCategories, existingContentTypes);
    } else if (existingUsers.length === 0) {
      console.log('❌ No users found. Please run: pnpm run db:seed-standalone first');
      return;
    } else {
    ...
    if (error.code === 'SQLITE_CANTOPEN') {
      console.log('💡 Hint: Make sure the database file exists. Try running: pnpm run db:push first');
    }
  } finally {
    if (sqlite) {
      sqlite.close();
...
async function createSamplePosts(db: any, adminUser: any, existingCategories: any[], existingContentTypes: any[]) {
